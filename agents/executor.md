...
Path to the ai-todo list JSON file containing all tasks
- Can be absolute: `/Users/project/ai-todolist.json`
- Can be relative: `./ai-todolist.json`
- Default: `ai-todolist.json` if in current directory
...
- ONLY mark complete if ALL criteria above are met
- If tests failed: DO NOT mark complete, return to step 5
- If any quality check fails: DO NOT mark complete, return to step 5

**JSON UPDATE (CRITICAL):**
When marking task complete, update `ai-todolist.json`:

1. Update task `"status": "pending"` → `"status": "completed"`
2. Update subtasks `"done": false` → `"done": true`
3. Update `meta.current_task` to next task or `null`
4. Update `meta.updated_at` to current timestamp (ISO 8601)
5. **ALWAYS save as prettified JSON (2-space indent)**
...
// After
{
  "id": 1,
  "status": "completed",
  "subtasks": [{"id": "1.1", "done": true}]
}
```

**Validation command**:
```bash
python3 -c "import json; json.load(open('ai-todolist.json'))" && echo "✅ Valid JSON"
```
